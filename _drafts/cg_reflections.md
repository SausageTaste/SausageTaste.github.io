...
[http://notiondraw.com](http://notiondraw.com)
...
여러분의 눈의 위치는 점 E라고 뒀을 때, $v = P - E$로 view 벡터를 구합니다.
여기서 $v$, $P$, $E$ 모두 3차원 벡터입니다.
그리고 물체의 표면으로부터 수직 방향을 가리키는 normal 벡터 $n$을 이용하는데, 이때 $\hat{n} = \frac{n}{||n||}$으로 정의합니다.
그러면 다음과 같은 공식을 이용하면 반사 벡터 $r$을 얻을 수 있습니다.

ma-th r = v - 2(v \cdot \hat{n})\hat{n} ma-th
...
아닙니다!
"반사 벡터가 가리키는 방향을 따라간다" 이 지극히 단순한 문장이 구체적으로 어떤 연산을 가리킬까요?
그렇죠.
레이트레이싱입니다.
벡터 $r$이라는 광선을 발사하여 빌딩 표면에 닿는 점 $A$를 계산한다, 정확하게 레이트레이싱이죠.
그리고 당연하게도 2004년 게임인 하프라이프 2에서 레이트레이싱을 사용할 수는 없었을 것입니다.

여기서 큐브맵이 등장할 차례입니다!
그림에서는 점 C가 바로 큐브맵 위치입니다.
그리고 C를 감싸는 상자는 C 주변 공간의 색깔 정보가 저장된 큐브맵 이미지를 나타냅니다.
큐브맵 반사에서는 C에서 출발하는 반사 벡터 $\hat{r}$을 이용해 큐브맵 색깔을 샘플링하여 반사상의 색깔을 가져옵니다.

그런데 $\hat{r}$의 연장선을 그어보면 $A’$를 가리키고 있습니다.
사실 정확한 반사상의 위치인 A를 찾으려면 \hat{a}을 사용해 큐브맵 이미지를 샘플링해야 되는데요.
레이트레이싱 없이는 A를 찾을 수 없으므로 \hat{a}도 찾을 수 없습니다.
